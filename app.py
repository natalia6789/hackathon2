...
if page == "Бизнес":
    ...
    col4.metric("Ожидаемая прибыль", f"{total_expected_margin:,.2f}")
        ...
        if not plot_data.empty:
            ...
            if not combined_data.empty:
                ...
                if show_forecast and forecast_df_clean is not None:
                    ...
                    #         with col2:
                    #             st.metric(
                    #                 label=f"Прогноз на {pd.to_datetime(forecast_after_year['date'].values[0]).strftime('%d.%m.%Y')}",
                    ...
                    with col_all2:
                        # Определяем цвет дельты (зеленый для роста, красный для падения)
                        delta_color = "normal"  # normal = зеленый для положительного, красный для отрицательного
                        st.metric(
...
### НЕДЕЛЬНАЯ МАРЖА ПО ИСТОЧНИКАМ
    ...
    def aggregate_activity_by_granularity(df, granularity):
        ...
        if granularity == "Неделя":
            ...
            aggregated = df['weekly_sessions'].resample('M').sum().reset_index()
        else:  # День
